...
export function FloatingMessenger({ currentUserId, currentUserName }: { currentUserId: string; currentUserName: string }) {
  ...
  // Tải danh sách học viên đang online để chat
    ...
    try {
      ...
      if (res.ok) {
        ...
        const list: Peer[] = data
          ...
          .filter((u: Peer) => u.id !== currentUserId);
        // Lọc trùng lặp
  ...
  // Tải lịch sử tin nhắn khi chọn một người
  React.useEffect(() => {
    if (!selectedPeer) return;

    fetch(`/api/messages?receiverId=${selectedPeer.id}`)
      .then((res) => res.json())
      .then((data) => setMessages(data || []));

    // Lắng nghe Realtime tin nhắn 1-1 mới
        ...
        <div className="mb-3 flex h-[460px] w-80 sm:w-96 flex-col overflow-hidden rounded-3xl border border-[#96AFEB]/20 bg-[#111A2E] shadow-2xl backdrop-blur-xl animate-in fade-in slide-in-from-bottom-5">
          ...
          <div className="flex items-center justify-between border-b border-[#96AFEB]/15 bg-[#0D1424] px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-xs font-bold text-white">
                {selectedPeer ? selectedPeer.displayName : "Messenger Bạn Học"}
              </h4>
            </div>
            <button onClick={() => setIsOpen(false)} className="text-xs text-[#8390AF] hover:text-white">✕</button>
          </div>
          ...
          {!selectedPeer ? (
                ...
                activePeers.map((peer) => (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <p className="truncate text-xs font-semibold text-white">{peer.displayName}</p>
            ...
            <div className="flex flex-1 flex-col overflow-hidden">
              ...
              {/* Ô gõ tin nhắn */}
              <form onSubmit={handleSend} className="border-t border-[#96AFEB]/15 bg-[#0D1424] p-2.5 flex gap-2">
                <input
                  type="text"
                  value={inputText}
                  onChange={(e) => setInputText(e.target.value)}
                  placeholder={`Nhắn cho ${selectedPeer.displayName}...`}
